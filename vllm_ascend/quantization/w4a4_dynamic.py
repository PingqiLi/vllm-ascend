...
def fused_experts(x,
    ...
    if w1_scale is not None:
        x_quantized, pertoken_scale = torch_npu.npu_dynamic_quant(expanded_x, dst_type=torch.quint4x2)
    ...
    if w2_scale is not None:
        ...
        if is_w8a8_w2:
             ...
             x_quantized, pertoken_scale = torch_npu.npu_dynamic_quant(expanded_x)
        else:
             x_quantized, pertoken_scale = torch_npu.npu_dynamic_quant(expanded_x, dst_type=torch.quint4x2)
...
class AscendW4A4DynamicLinearMethod:
    ...
    def apply(
        ...
        input_shape = x.shape
        original_dtype = x.dtype
        x_quantized_reshaped, pertoken_scale = torch_npu.npu_dynamic_quant(x, dst_type=torch.quint4x2)
...
class AscendW4A4DynamicFusedMoEMethod:
    ...
    def process_scale(self, weight: torch.Tensor, scale, per_group_scale):
        ...
        if self.is_per_channel_weight:
            # Matches w4a4_flatquant_dynamic_reference.py convert_scales logic
            E, _, N = scale.shape
            scale_fp32 = scale.view(E, N).to(torch.float32)
            target_shape = (E, 1, N)
        else:
            # Handle group quantization
            per_group_scale = per_group_scale.transpose(1, 2).contiguous()
            group_num, k, n = weight.shape
            # Adjust n based on packing stride (consistent with original logic)
            n = n * 2 
            per_group_scale = per_group_scale.reshape(group_num, -1, n)
            group_num, quantgroup_num, n = per_group_scale.shape
            
            # Broadcast multiply to get final float scales
            scale_fp32 = (scale * per_group_scale).to(torch.float32)
            target_shape = (group_num, quantgroup_num, n)

        # Common packing logic: interleave float32 bits with zeros to form int64
        # 1. View float32 as int32
        scale_int32 = scale_fp32.view(torch.int32)
        # 2. Create zeros for interleaving
        zeros = torch.zeros_like(scale_int32)
        # 3. Stack to interleave: [s0, 0, s1, 0, ...]
        packed = torch.stack((scale_int32, zeros), dim=-1).view(*scale_int32.shape[:-1], -1)
        # 4. View as int64 and reshape to expected target shape
        scale_uint64 = packed.view(torch.int64).view(*target_shape)
        
        return scale_uint64, None
